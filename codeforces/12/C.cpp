...
void task(){
    ...
    std::map<string, int> Mapmx, Mapmn, Map;
    for(int i = 0; i<m; i++){
        cin >> u[i];
        Map[u[i]]++;
    }
    
    std::vector<pair<int, string>>sortMap;
    for(auto x:Map){
        sortMap.emplace_back(x.second, x.first);
    }
    sort(all(sortMap));
    reverse(all(sortMap));
    ...
    sort(all(v));
    std::vector<int> maxv = v;
    reverse(all(maxv));
    
    for(auto c: sortMap){
        string fruits = c.second;
        Mapmx[fruits] = v.back();
        v.pop_back();

        Mapmn[fruits] = maxv.back();
        maxv.pop_back();
    }

    int mn = 0, mx = 0;
    for(auto x: Map){
        mn += x.second * Mapmn[x.first];
        mx += x.second * Mapmx[x.first];
    }
    cout << mn <<' '<<mx<<endl;
}
